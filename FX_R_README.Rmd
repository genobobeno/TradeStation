...
R looks into the working directory each time you make a call to a function, e.g. `GetPriceData()`; in other words, R will run `readRDS(MyAccount.rds)` and  `readRDS(MyToekn.rds)` each time it makes an API call for the price data. For my code, I have set this up to use my practice account... and I have not uploaded those to github, so you need to go get your own.
...
Font sizes might be rough here if you're on a small screen... but you should get the picture. Looking at the top table: USD_JPY is impulsive and uptrending on three consecutive time frames. A UTS (Up-Trend-Setup) would tell me that I should go open my chart and look for a trade. 
...
```{r, echo=TRUE,out.width = '100%',fig.height=8,fig.width=10}
suppressMessages(IND<-GetIndexes(Price=GetAllPrices(Minutes = 1,LookBackHours = 7),pca.plot = TRUE))
```
The S&P fell off a cliff this morning, so risk-off down the line. USD has been oversold, so it's leading the pack.
