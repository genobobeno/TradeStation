...
InstrumentsList <- function(AccountType, Token, AccountID, ApiVersion="v2") {
  #AccountType=OA_At; Token=OA_Ak; AccountID=OA_Ai; ApiVersion="v3"
...
ActualPrice <- function(AccountType,Token,Instrument)
{
  ...
  auth      <- c(Authorization = paste("Bearer",Token,sep=" "))
  Queryhttp <- paste(httpaccount,"/v3/prices?instruments=",sep="")
...
HisSpreads <- function(AccountType,Granularity,DayAlign,TimeAlign,Token,Instrument,
  ...
  if(!is.null(Count)) {
    ...
    auth           <- c(Authorization = paste("Bearer",Token,sep=" "))
    QueryHistPrec  <- paste(httpaccount,"/v3/candles?instrument=",sep="")
    ...
    qstart <- paste("from=",Start,sep="")
    qend   <- paste("to=",End,sep="")
...
HisPrices  <- function(Account,AccountType,Granularity,DayAlign,TimeAlign,
  ...
  # Start<-str_replace(as.character(OA_F1Px)," ","T")
  # End<-str_replace(as.character(OA_F2Px)," ","T")
  if(AccountType == "practice"){
    httpaccount  <- "https://api-fxpractice.oanda.com"
  } else if(AccountType == "live"){
      httpaccount  <- "https://api-fxtrade.oanda.com"
  ...
  if(!is.null(Count)) {
    ...
    auth           <- c(Authorization = paste("Bearer",Token,sep=" "))
    QueryHistPrec  <- paste(httpaccount,"/v3/accounts/",Account,"/instruments/",Instrument,"/candles?",sep="")
    #QueryHistPrec1 <- paste(QueryHistPrec,Instrument,sep="")
    ...
    qgranularity   <- paste("granularity=",Granularity,sep="")
    qdailyalignment    <- paste("dailyAlignment=",DayAlign,sep="")
    #qalignmenttimezone    <- paste("alignmentTimezone=","GMT",sep="")
    
    QueryHistPrec2 <- paste(QueryHistPrec,qstart,qend,qgranularity,  #,qcandleFormat
                            qdailyalignment,sep="&")
  }
  ...
  Prices        <- data.frame(InstHistPjson[[3]])
  Prices$time <- paste(substr(Prices$time,1,10),substr(Prices$time,12,19), sep=" ")
  Prices$Open<-Prices$mid[,1];Prices$High<-Prices$mid[,2]
  Prices$Low<-Prices$mid[,3];Prices$Close<-Prices$mid[,4]
  Prices$mid<-NULL
  colnames(Prices) <- c("Complete","TickVolume","TimeStamp","Open","High","Low","Close")
  Prices$TimeStamp <- as.POSIXct(strptime(Prices$TimeStamp, "%Y-%m-%d %H:%M:%OS"),
                                 origin="1970-01-01",tz = "UTC")
  attributes(Prices$TimeStamp)$tzone <- TimeAlign
...
UsersAccounts <- function(AccountType,Token,UserName)
{
  ...
  auth       <- c(Authorization = paste("Bearer",Token,sep=" "))
  Queryhttp  <- paste(httpaccount,"/v3/accounts?username=",sep="")
...
AccountOrders  <- function(AccountType,AccountID,Token,Instrument,ApiVersion="v2") {
...
OrderBook <- function(AccountType,Token,Instrument,Period)
{
  ...
  Queryhttp  <- paste(httpaccount,"/labs/v3/orderbook_data?instrument=",sep="")
...
OpenTrades  <- function(AccountType,AccountID,Token,Instrument,ApiVersion="v2") {
...
AccountHistTransactions  <- function(AccountType,AccountID,Token,Instrument,Count,ApiVersion="v2") {
...
EconomicCalendar <- function(AccountType,Token,Instrument,Period)
{
  ...
  Queryhttp  <- paste(httpaccount,"/labs/v3/calendar?instrument=",sep="")
  Queryhttp1 <- paste(Queryhttp,Instrument,sep="")  
  Queryhttp2 <- paste(Queryhttp1,"period=",sep="&")  
  Queryhttp3 <- paste(Queryhttp2,Period,sep="")
...
RatiosPosturas  <- function(AccountType,Token,Instrument,Period)
{
  ...
  Queryhttp  <- paste(httpaccount,"/labs/v3/historical_position_ratios?instrument=",sep="")
  Queryhttp1 <- paste(Queryhttp,Instrument,sep="")
  Queryhttp2 <- paste(Queryhttp1,"period=",sep="&")  
  Queryhttp3 <- paste(Queryhttp2,Period,sep="")                   
...
Spreads <- function(AccountType,Token,Instrument,Period)
{
  ...
  Queryhttp  <- paste(httpaccount,"/labs/v3/spreads?instrument=",sep="")
  Queryhttp1 <- paste(Queryhttp,Instrument,sep="")
  Queryhttp2 <- paste(Queryhttp1,"period=",sep="&")
  Queryhttp3 <- paste(Queryhttp2,Period,sep="")
...
Autochartist_Inst <- function(AccountType,Token,Instrument,Period,Type)
{
  ...
  Queryhttp  <- paste(httpaccount,"/labs/v3/signal/autochartist?instrument=",sep="")
  Queryhttp1 <- paste(Queryhttp,Instrument,sep="")
...
LiveRates <- function(AccountType, AccountID, Token, Inst)
{
  ...
  http  <- paste(httpaccount,"/v3/prices?accountid=", AccountID, "&instruments=",Inst,sep="")
